...
function deletePrinter(name) {
  if (printerHasJob(name) === 0) {
    ...
    console.log(
      `Error: Print queue ${name} has pending jobs. clear the jobs and retry`
    );
  }
}
...
function addTCPIPPort(portName) {
  ...
  execSync(`c:\\windows\\system32\\cscript.exe ./lib/prnport.vbs ${args}`);
  return 1;
}
...
function addDriver(prnQueueOptions, osArch) {
  if (fs.existsSync(prnQueueOptions.infAbsPath)) {
    ...
    return MyLibrary.AddPrintDriver(
      prnQueueOptions.infAbsPath,
      prnQueueOptions.driverName,
      arch
    );
  } else {
    return -1;
  ...
  if (drvChk !== -1 || drvChk !== -2) {
    if (addTCPIPPort(prnQueueOptions.portName) === 1) {
      if (
        ...
        console.log(
          `Error: Print queue "${prnQueueOptions.queueName}" could not be added`
        );
      }
    } else
      console.log(
        `Error: Port "${prnQueueOptions.portName}" could not be added`
      );
  }
}
